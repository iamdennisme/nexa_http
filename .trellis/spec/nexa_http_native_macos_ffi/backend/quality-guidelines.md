...
- arm64/x64 必须由 canonical target matrix 显式驱动各自 Rust target，输出到不同的 Flutter hook target-scoped directory。
...
- 不要把 macOS build artifact 路径写死进 Rust crate、Podspec 或 carrier package；路径由 canonical target matrix 和 Flutter hook output contract 管理。
