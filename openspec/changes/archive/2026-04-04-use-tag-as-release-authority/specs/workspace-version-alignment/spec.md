...
The system SHALL support inspecting whether the seven release-train packages use the same semantic version for coordinated maintenance, but this alignment check SHALL remain advisory metadata hygiene rather than a required gate for tag-triggered release publication. The check SHALL exclude `packages/nexa_http/example`.
