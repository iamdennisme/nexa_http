...
- Android runtime marker采集必须复用唯一一次`flutter build apk --release`的`app-release.apk`：path/candidate与released fixture在build前通过同一个配置器让`android/app/src/main/AndroidManifest.xml`包含恰好一条`android.permission.INTERNET`，build阶段注入`127.0.0.1` fixture URL，runtime按`adb install -t -r`、清空logcat、一次`adb reverse tcp:<fixture-port> tcp:<fixture-port>`、`adb shell am start -W`启动；reverse端口直接来自fixture URL且必须在Activity启动前建立。不得依赖emulator特殊宿主地址、只存在于debug/profile manifest的权限，不得调用`flutter run`、二次Gradle assemble或直接启动debug APK。随后只对同device本轮`flutter:I`日志做最多60次有界轮询；Actions已观测到真实ATD callback在第30次之后到达，仍要求唯一完整marker，超出上限直接失败，proof判定后best-effort force-stop fixture。
