...
- Android runtime marker采集必须复用唯一一次`flutter build apk`的输出：build阶段注入fixture URL，runtime按`adb install -t -r`、清空logcat、`adb shell am start -W`启动，不得再次调用`flutter run`或Gradle assemble。随后只对同device本轮`flutter:I`日志做最多60次有界轮询；Actions已观测到真实ATD callback在第30次之后到达，仍要求唯一完整marker，超出上限直接失败，proof判定后best-effort force-stop fixture。
