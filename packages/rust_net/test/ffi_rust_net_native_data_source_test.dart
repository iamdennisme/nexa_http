...
void main() {
  test('executes through async ffi and decodes raw response bytes', () async {
    final bindings = _FakeRustNetBindings(
      onExecuteAsync: ({
        required int clientId,
        required int requestId,
        required String requestJson,
        required Uint8List bodyBytes,
        required RustNetExecuteCallback callback,
      }) {
        expect(clientId, 7);
        expect(
        ...
        final resultPointer = calloc<RustNetBinaryResult>();
        resultPointer.ref
          ..is_success = 1
          ..status_code = 200
        ...
        callback
            .asFunction<void Function(int, ffi.Pointer<RustNetBinaryResult>)>()(
          requestId,
          resultPointer,
        );
        return 1;
    ...
    final dataSource = FfiRustNetNativeDataSource(
      libraryPath: 'test-only',
      bindings: bindings,
    );
...
class _FakeRustNetBindings extends RustNetBindings {
  _FakeRustNetBindings({required this.onExecuteAsync})
  ...
  @override
  void rust_net_binary_result_free(ffi.Pointer<RustNetBinaryResult> value) {
    freedResultCount += 1;
