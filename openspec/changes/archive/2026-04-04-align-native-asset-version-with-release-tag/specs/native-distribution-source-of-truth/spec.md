...
- **AND** it SHALL NOT redefine the same target, artifact, or release-identity mapping locally

#### Scenario: Release-consumer lookup runs from a tagged git dependency
- **WHEN** distribution logic resolves the manifest URI for a consumer pinned to a tag such as `v0.0.1`
- **THEN** it SHALL request `nexa_http_native_assets_manifest.json` from that same tag path
- **AND** it SHALL treat that selected tag as the only semantic release identity for the lookup
- **AND** it SHALL NOT derive the release URL from unrelated local package version metadata
