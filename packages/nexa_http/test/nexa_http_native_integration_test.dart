...
void main() {
  group(
    'NexaHttpClient native integration',
    () {
      ...
      test('supports DELETE, HEAD, and OPTIONS', () async {
        ...
        final optionsResponse = await _execute(
          ...
          RequestBuilder()
              ...
              .method('OPTIONS', RequestBody.takeBytes(Uint8List(0)))
              .build(),
        );
      ...
      test(
        ...
        () async {
          ...
          for (final entry in expectations.entries) {
            ...
            expect(response.finalUrl, expectedFinalUri);
            final responseJson =
                jsonDecode(await response.body!.string())
    ...
    tags: <String>['native-integration', 'macos'],
  );
}
