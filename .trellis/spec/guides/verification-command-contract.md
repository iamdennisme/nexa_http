...
- clean-host fixture在proof前必须输出固定顺序的`NEXA_HTTP_RUNTIME_PHASE`诊断，并在catch时把JSON编码的`NEXA_HTTP_RUNTIME_FAILURE`写到stdout；typed HTTP failure必须包含kind/uri/diagnostics，native network/timeout diagnostics保留source chain。零proof失败包含本轮去重phase与failure，诊断本身不改变gate结论，也不得作为兼容成功路径。
