...
- `MacosProxySource` 只拥有 SystemConfiguration raw-value adapter，refresh mode 保持 `ConstructionBoundary`。
...
- `cargo fmt --all --check`
- `cargo clippy --no-deps -p nexa_http_native_macos_ffi --all-targets -- -D warnings`
