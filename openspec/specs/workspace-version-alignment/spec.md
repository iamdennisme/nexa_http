...
The system SHALL verify that repository release tags resolve to the same semantic version declared by the seven aligned workspace packages, while treating those workspace package versions as advisory metadata for `release-consumer` native asset lookup rather than the authoritative release identity.
...
- **AND** they SHALL see that tag-authoritative `release-consumer` native asset lookup does not fall back to workspace package versions as its release identity
