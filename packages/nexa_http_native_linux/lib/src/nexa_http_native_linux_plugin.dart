import 'dart:ffi';
import 'dart:io';

import 'package:nexa_http/src/loader/nexa_http_platform_registry.dart';
import 'package:path/path.dart' as p;

final class NexaHttpNativeLinuxPlugin {
  NexaHttpNativeLinuxPlugin._();

  static void registerWith() {
    NexaHttpPlatformRegistry.instance ??= const _NexaHttpNativeLinuxRuntime();
  }
}

final class _NexaHttpNativeLinuxRuntime implements NexaHttpNativeRuntime {
  const _NexaHttpNativeLinuxRuntime();

  static const _environmentVariable = 'NEXA_HTTP_NATIVE_LINUX_LIB_PATH';
  static const _libraryFileName = 'libnexa_http_native.so';

  @override
  DynamicLibrary open() {
    final explicitPath = Platform.environment[_environmentVariable];
    if (explicitPath != null && explicitPath.trim().isNotEmpty) {
      return DynamicLibrary.open(explicitPath.trim());
    }

    final resolvedPath = _discoverPackagedLibrary() ?? _discoverWorkspaceLibrary();
    if (resolvedPath != null) {
      return DynamicLibrary.open(resolvedPath);
    }

    throw StateError(
      'Unable to locate the nexa_http Linux native library. '
      'Build nexa_http_native_linux or set $_environmentVariable.',
    );
  }

  String? _discoverPackagedLibrary() {
    final seeds = <String>{
      Directory.current.path,
      p.dirname(Platform.resolvedExecutable),
    };

    for (final seed in seeds) {
      var current = p.normalize(seed);
      while (true) {
        final candidate = p.join(
          current,
          'linux',
          'Libraries',
          _libraryFileName,
        );
        if (File(candidate).existsSync()) {
          return p.normalize(candidate);
        }

        final parent = p.dirname(current);
        if (parent == current) {
          break;
        }
        current = parent;
      }
    }
    return null;
  }

  String? _discoverWorkspaceLibrary() {
    final seeds = <String>{
      Directory.current.path,
      p.dirname(Platform.resolvedExecutable),
    };

    for (final seed in seeds) {
      var current = p.normalize(seed);
      while (true) {
        for (final mode in <String>['debug', 'release']) {
          final candidate = p.join(
            current,
            'target',
            mode,
            'libnexa_http_native_linux_ffi.so',
          );
          if (File(candidate).existsSync()) {
            return p.normalize(candidate);
          }

          final legacyCandidate = p.join(
            current,
            'packages',
            'nexa_http_native_linux',
            'native',
            'nexa_http_native_linux_ffi',
            'target',
            mode,
            'libnexa_http_native.so',
          );
          if (File(legacyCandidate).existsSync()) {
            return p.normalize(legacyCandidate);
          }
        }

        final parent = p.dirname(current);
        if (parent == current) {
          break;
        }
        current = parent;
      }
    }
    return null;
  }
}
