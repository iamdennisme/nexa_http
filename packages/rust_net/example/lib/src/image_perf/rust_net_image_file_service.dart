...
final class RustNetImageFileService extends FileService {
  RustNetImageFileService({
    HttpExecutor? executor,
    this.onSample,
    RustNetClientConfig config = const RustNetClientConfig(
      timeout: Duration(seconds: 20),
      userAgent: 'rust_net_example/1.0.0',
    ),
  })  : _ownsExecutor = executor == null,
        _executor = executor ?? RustNetClient(config: config);
  ...
  String? _header(RustNetResponse response, String name) {
  ...
  int? _contentLengthFromHeaders(RustNetResponse response) {
  ...
  DateTime _validTillFromResponse(RustNetResponse response) {
  ...
  String _fileExtensionFromResponse(RustNetResponse response) {
