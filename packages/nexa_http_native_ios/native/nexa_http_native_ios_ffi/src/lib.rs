...
impl PlatformRuntimeState for IosPlatformRuntime {
...
fn spawn_proxy_refresh_worker(state: Arc<ProxyRuntimeState>) {
    let _ = thread::Builder::new()
        .name("nexa-http-ios-proxy".to_string())
        .spawn(move || {
            loop {
...
static RUNTIME: Lazy<NexaHttpRuntime<IosPlatformRuntime>> =
    Lazy::new(|| NexaHttpRuntime::new(IosPlatformRuntime::new()));
...
pub extern "C" fn nexa_http_binary_result_free(value: *mut NexaHttpBinaryResult) {
    NexaHttpRuntime::<IosPlatformRuntime>::binary_result_free(value);
}
