...
import '../hook/build.dart' as rust_net_native_linux_build_hook;
...
void main() {
  test('emits the packaged Linux native library when present', () async {
    ...
    await testCodeBuildHook(
      mainMethod: rust_net_native_linux_build_hook.main,
      targetOS: OS.linux,
      ...
      check: (input, output) async {
        ...
        expect(
          asset.id,
          'package:rust_net_native_linux/src/native/rust_net_native_ffi.dart',
        );
        expect(asset.linkMode, isA<DynamicLoadingBundled>());
        expect(asset.file, isNotNull);
        expect(
          File.fromUri(asset.file!).path,
          endsWith('linux/Libraries/librust_net_native.so'),
        );
      },
    );
  });
