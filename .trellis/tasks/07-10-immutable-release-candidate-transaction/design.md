...
- Android：`aosp_atd` x64 emulator，真实candidate Android asset runtime。
...
- Android gate的clean-host APK只执行一次`flutter build apk --release`；runtime直接`adb install`并启动`app-release.apk`，不使用debug VM attach，也不再用`flutter run`触发第二次Gradle assemble。
