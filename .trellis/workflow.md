...
- Flutter SDK integration tasks must cite `.trellis/spec/guides/flutter-sdk-authoring-contract.md` and map the work against host integration surface, hidden internal packages, native lifecycle ownership, formal configuration, failure reporting, and clean-host acceptance.
...
Flutter SDK/native packaging changes: read `.trellis/spec/guides/flutter-sdk-authoring-contract.md` and add the contract mapping before start.
...
- 2.2 Quality check `[required · repeatable]` (Flutter SDK changes must check the authoring contract, not just tests)
...
Flutter SDK/native packaging changes: verify `.trellis/spec/guides/flutter-sdk-authoring-contract.md`; host integration must remain package dependencies + public `package:nexa_http/nexa_http.dart` API + standard Flutter build, with no undocumented native-project workaround.
Release-bound Flutter SDK work: run or explicitly schedule the Flutter SDK Release Gate before Phase 3.4 commit.
[/workflow-state:in_progress]
...
### Flutter SDK Release Gate

Before publishing a release or blessing a release candidate, run clean-host acceptance for each targeted host platform. At minimum, a freshly created Flutter app must add the SDK dependency, run `flutter pub get`, call the public `nexa_http` API, and pass `flutter build` or `flutter run` while proving plugin registration and native artifact packaging.

Use the repository commands where applicable:
...
If this gate fails for a target platform, the release is blocked unless the release explicitly drops that platform.
...
- For Flutter SDK/native packaging changes, load `.trellis/spec/guides/flutter-sdk-authoring-contract.md` and write the host integration boundary, native lifecycle ownership, failure reporting, and clean-host acceptance mapping into the planning artifacts before start
...
For Flutter SDK/native packaging changes, the check must also load
`.trellis/spec/guides/flutter-sdk-authoring-contract.md` and verify:
- host runtime examples import only `package:nexa_http/nexa_http.dart`
- host setup stays inside package dependencies plus the standard Flutter build chain
- carrier/internal packages do not leak into host runtime code
- native artifact download, verification, cache, registration, and packaging stay SDK-owned
- any mirror/offline/debug/enterprise path uses a formal documented configuration surface
- failure messages report stage, platform, architecture, expected host action, SDK version/ref, and underlying error
- clean-host verification has run or is explicitly scheduled for unavailable platforms
...
**Flutter SDK release preamble**: for release-bound SDK changes, run the Flutter SDK Release Gate from the Phase Index before committing, or record exactly which platform runner must cover any unavailable target. A failing target blocks release unless the release explicitly drops that platform.
