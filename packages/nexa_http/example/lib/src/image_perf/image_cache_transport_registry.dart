...
final class ImageCacheTransportRegistry {
  ImageCacheTransportRegistry({
    ...
    ImageCacheManagerFactory? rustNetFactory,
        ...
        _rustNetFactory = rustNetFactory ?? _createNexaHttpManager;
  ...
  static BaseCacheManager _createNexaHttpManager({
    ImageRequestSampleCallback? onSample,
  }) {
    return _InstrumentedImageCacheManager(
      cacheKey: 'rustNetExampleNexaHttpImages',
      fileService: NexaHttpImageFileService(onSample: onSample),
    );
  }
}
...
final class _InstrumentedImageCacheManager extends CacheManager
  ...
  @override
  Future<void> dispose() async {
    if (_fileService case final NexaHttpImageFileService rustNetFileService) {
