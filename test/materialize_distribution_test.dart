...
void main() {
  test(
    'materializes selected packages and their local path dependencies',
    () async {
      ...
      await _writeFile(
        workspace,
        'packages/nexa_http_native_macos/lib/nexa_http_native_macos.dart',
        'library nexa_http_native_macos;\n',
      );
      await _writeFile(
        workspace,
      ...
      expect(File(p.join(outputDir.path, 'pubspec.yaml')).existsSync(), isTrue);
      expect(
        File(
          p.join(
            outputDir.path,
      ...
      expect(
        File(
          p.join(outputDir.path, 'packages', 'nexa_http', 'pubspec.yaml'),
        ).existsSync(),
        isTrue,
      );
      expect(
  ...
  test('does not require carrier-owned native binaries', () async {
    final workspace = await Directory.systemTemp.createTemp(
