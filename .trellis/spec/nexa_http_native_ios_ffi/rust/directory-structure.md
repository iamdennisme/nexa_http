...
- `src/lib.rs` 只定义 iOS `RUNTIME` 和 runtime wiring，并调用 core `export_nexa_http_ffi!` 生成统一 C ABI exports。
...
- `IosProxySource` 只把当前 SystemConfiguration 值映射为内存结构；proxy snapshot 由 core `ManagedProxyState` 持有。
- runtime、client 和 request lifecycle 由 `nexa_http_native_core` 管理，本 crate 不创建第二份 registry。
- 测试只构造 raw Apple proxy values，不写入系统配置或 production 文件。

## 真实例子

- `packages/nexa_http_native_ios/native/nexa_http_native_ios_ffi/src/lib.rs`：保留 construction-boundary runtime 并调用共享 ABI export macro。
