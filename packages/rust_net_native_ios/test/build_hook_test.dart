...
import '../hook/build.dart' as rust_net_native_ios_build_hook;
...
void main() {
  test('emits the packaged iOS simulator native library when present', () async {
    ...
    final libraryFile = File(
      '${frameworksDir.path}/librust_net_native-ios-sim-arm64.dylib',
    );
    ...
    await testCodeBuildHook(
      mainMethod: rust_net_native_ios_build_hook.main,
      targetOS: OS.iOS,
      ...
      check: (input, output) async {
        ...
        expect(
          asset.id,
          'package:rust_net_native_ios/src/native/rust_net_native_ffi.dart',
        );
        expect(asset.linkMode, isA<DynamicLoadingBundled>());
        expect(asset.file, isNotNull);
        expect(
          File.fromUri(asset.file!).path,
          endsWith('ios/Frameworks/librust_net_native-ios-sim-arm64.dylib'),
        );
      },
    );
  });
